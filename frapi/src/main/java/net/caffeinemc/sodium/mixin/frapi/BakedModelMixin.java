...
public interface BakedModelMixin extends FabricBlockStateModel {
    @Override
    default void emitQuads(QuadEmitter e, BlockAndTintGetter blockView, BlockPos pos, BlockState state, RandomSource random, Predicate<@Nullable Direction> cullTest) {
        QuadEmitter emitter;
        ...
        if (emitter instanceof AbstractBlockRenderContext.BlockEmitter be) {
            List<BlockModelPart> parts = PlatformModelAccess.getInstance().collectPartsOf((BlockStateModel) this, blockView, pos, state, random, be); // TODO
            ...
            for (int i = 0; i < partCount; ++i) {
                ((FabricBlockModelPart) parts.get(i)).emitQuads(e, cullTest);
            }
        } else {
            ...
            for(int i = 0; i < partCount; ++i) {
                ((BlockModelPart)parts.get(i)).emitQuads(e, cullTest);
            }
