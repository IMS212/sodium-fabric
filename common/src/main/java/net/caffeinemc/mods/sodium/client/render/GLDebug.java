...
public final class GLDebug {
    ...
    public static int setupDebugMessageCallback(PrintStream stream) {
        ...
        if (caps.OpenGL43) {
            SodiumClientMod.logger().info("[GL] Using OpenGL 4.3 for error logging.");
            ...
            if ((GL43C.glGetInteger(33310) & 2) == 0) {
                SodiumClientMod.logger().warn("[GL] Warning: A non-debug context may not produce any debug output.");
                GL43C.glEnable(37600);
                return 2;
            }
            return 1;
        } else if (caps.GL_KHR_debug) {
            SodiumClientMod.logger().info("[GL] Using KHR_debug for error logging.");
            GLDebugMessageCallback proc = GLDebugMessageCallback.create((source, type, id, severity, length, message, userParam) -> {
                stream.println("[LWJGL] OpenGL debug message");
                printDetail(stream, "ID", String.format("0x%X", id));
                printDetail(stream, "Source", getDebugSource(source));
            ...
            SodiumClientMod.logger().info("[GL] Using ARB_debug_output for error logging.");
            ...
            SodiumClientMod.logger().info("[GL] Using AMD_debug_output for error logging.");
            ...
            SodiumClientMod.logger().info("[GL] No debug output implementation is available, cannot return debug info.");
            return 0;
        }
    }
    ...
    public static int disableDebugMessages() {
        ...
        if (caps.OpenGL43) {
            ...
            SodiumClientMod.logger().info("[GL] No debug output implementation is available, cannot disable debug info.");
            return 0;
        }
    }
