...
const uint CHUNK_Y_SIZE = 24;

uint threeToOne(ivec3 coord) {
    return coord.x + coord.y * 16 + coord.z * 16 * CHUNK_Y_SIZE;
}

uint threeToOneBlock(ivec3 coord) {
    return coord.x + coord.y * 16 + coord.z * 16 * 16;
}

const ivec3 CHUNK_BOUNDS = ivec3(12, 24, 12);
...
// 3 4 0
    int chunkBuffer = chunkBuffer[chunkLocations[threeToOne(ivec3(627, 4, 624) % CHUNK_BOUNDS)]].voxels[threeToOneBlock(ivec3(0, 0, 0))].y >> 20 & 15;
    ...
    v_Color = vec4(max(vec4(0.2), vec4(chunkBuffer)).rgb, 1.0) * _vert_color;
