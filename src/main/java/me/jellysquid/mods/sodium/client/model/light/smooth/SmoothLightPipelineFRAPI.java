 ...
 */
public class SmoothLightPipelineFRAPI implements LightPipelineFRAPI {
    /**
    ...
    public SmoothLightPipelineFRAPI(LightDataAccess cache) {
    ...
    @Override
    public void calculate(QuadViewImpl quad, BlockPos pos, QuadLightData out, Direction cullFace, Direction lightFace, boolean shade) {
     ...
     * Flags: IS_ALIGNED, IS_PARTIAL
     */
    private void applyAlignedPartialFace(AoNeighborInfo neighborInfo, QuadView quad, BlockPos pos, Direction dir, QuadLightData out) {
        for (int i = 0; i < 4; i++) {
    ...
    /**
     * This method is the same as {@link #applyNonParallelFace(AoNeighborInfo, QuadView, BlockPos, Direction,
     ...
     */
    private void applyParallelFace(AoNeighborInfo neighborInfo, QuadView quad, BlockPos pos, Direction dir, QuadLightData out) {
        for (int i = 0; i < 4; i++) {
            // Clamp the vertex positions to the block's boundaries to prevent weird errors in lighting
            float cx = clamp(quad.x(i));
            float cy = clamp(quad.y(i));
     ...
     * Flags: !IS_ALIGNED, !IS_PARALLEL
     */
    private void applyNonParallelFace(AoNeighborInfo neighborInfo, QuadView quad, BlockPos pos, Direction dir, QuadLightData out) {
        for (int i = 0; i < 4; i++) {
            // Clamp the vertex positions to the block's boundaries to prevent weird errors in lighting
            float cx = clamp(quad.x(i));
            float cy = clamp(quad.y(i));
