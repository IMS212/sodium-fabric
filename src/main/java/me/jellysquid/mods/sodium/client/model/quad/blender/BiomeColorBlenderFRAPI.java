...
public final class BiomeColorBlenderFRAPI {
    ...
    public BiomeColorBlenderFRAPI(int radius) {
     ...
     */
    public <T> int[] getColors(BlockRenderView world, BlockPos origin, QuadView quad, ColorSampler<T> sampler, T state) {
    ...
    private <T> int getVertexColor(BlockRenderView world, BlockPos origin, QuadView quad, ColorSampler<T> sampler, T state,
                                   int vertexIdx) {
        // Offset the position by -0.5f to align smooth blending with flat blending.
        final float posX = quad.x(vertexIdx) - 0.5f;
        final float posY = quad.y(vertexIdx) - 0.5f;
        final float posZ = quad.z(vertexIdx) - 0.5f;
        ...
        final int c00 = this.getBlockColor(world, state, sampler, worldIntX + 0, worldIntY, worldIntZ + 0, quad.colorIndex());
        final int c01 = this.getBlockColor(world, state, sampler, worldIntX + 0, worldIntY, worldIntZ + 1, quad.colorIndex());
        final int c10 = this.getBlockColor(world, state, sampler, worldIntX + 1, worldIntY, worldIntZ + 0, quad.colorIndex());
        final int c11 = this.getBlockColor(world, state, sampler, worldIntX + 1, worldIntY, worldIntZ + 1, quad.colorIndex());
