...
public interface TerrainVertexSink extends VertexSink {
     ...
     */
    void writeVertex(float posX, float posY, float posZ, int color, float u, float v, int light, byte block_state);

    default void writeVertex(Vec3i offset, float posX, float posY, float posZ, int color, float u, float v, int light, byte block_state) {
        this.writeVertex(offset.getX() + posX, offset.getY() + posY, offset.getZ() + posZ, color, u, v, light, block_state);
    }
